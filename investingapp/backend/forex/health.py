    ...
    if success:
        ...
        if response_ms is not None:
            health.avg_response_ms = (
                int((health.avg_response_ms + response_ms) / 2) if health.avg_response_ms else response_ms
            )
        ...
        health.status = (
            ForexProviderHealth.Status.DEGRADED if status_code == 429 else ForexProviderHealth.Status.DOWN
        )
        if status_code == 429:
